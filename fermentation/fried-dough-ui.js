...
(() => {
  ...
  const DEFAULTS={
    ...
    initial_power:7,frying_power:5,recovery_power:6,lower_power:4,mixing_batches:2,oil_percent:3,sugar_percent:0.8
  };
  ...
  function createPanel(){
    const panel=document.createElement("div");panel.id="friedDoughPanel";panel.className="full fried-box";panel.hidden=true;panel.innerHTML=`
      ...
      <div class="fried-grid">
        ...
        <div><label for="friedMixingBatches">Impasti separati</label><input id="friedMixingBatches" type="number" min="1" max="5" step="1" value="2"></div>
  ...
  function rebuildFried(packet){
    ...
    const activeWindowMinutes=12+18*profile.mixing_batches+baselineBulk+baselineProof+profile.preheat_minutes+cookMinutes+5;
    const effectiveHours=Math.max(4,activeWindowMinutes/60);
  ...
  async function init(){
    await waitFor(()=>document.querySelector("#style")&&document.querySelector("#ovenType")&&document.querySelector("#generate")&&window.CucinaHubDoughSizingWizard&&window.CucinaHubDoughSizingEngine&&window.CucinaHubChatRecipeImportEngine?.VERSION===2&&window.CucinaHubSessionWorkflowBuilder?.buildFried&&typeof presets!=="undefined"&&typeof appliances!=="undefined");
    ...
    window.CucinaHubFriedDoughWizard={VERSION:1,DEFAULTS,readProfile,rebuildFried,syncPacket};
  }
